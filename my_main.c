...
void first_pass() {
	...
	for (i = 0; i < lastop; i++) {
		switch (op[i].opcode) {
			case FRAMES:
				printf("\n# of frames: %4.0f",op[i].op.frames.num_frames);
				num_frames = op[i].op.frames.num_frames;
				frames_found = 1;
				break;
			case BASENAME:
				printf("\nBasename: %s",op[i].op.basename.p->name);
				strncpy(name, op[i].op.basename.p->name, sizeof(name));
				break;
			case VARY:
				printf("\nVary: %4.0f %4.0f, %4.0f %4.0f",op[i].op.vary.start_frame,op[i].op.vary.end_frame,op[i].op.vary.start_val,op[i].op.vary.end_val);
				vary_found = 1;
				break;
		}
	}
	...
	if (name[0] == 0 && num_frames > 1) {
		char basename[10] = "sample";
		printf("\nDefault basename used is sample.\n");
	...
	for (i = 0; i < lastop; i++) {
		if (op[i].opcode == VARY) {
			...
			for (f = start_frame; f <= end_frame; f++) {
				...
				new_knob->value = start_val + (f - start_frame) * d;
				new_knob->next = knobs[f];
				knobs[f] = new_knob;
			}
		}
	}
